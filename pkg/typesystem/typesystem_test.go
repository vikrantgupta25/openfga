...
func TestGetLowestWeightEdgeForExclusion(t *testing.T) {
	t.Run("direct_edge_for_exclusion", func(t *testing.T) {
		...
		baseEdges, exclusionEdge, err := typeSystem.GetLowestWeightEdgeForExclusion(rootExclusionNode.GetUniqueLabel(), "user")
		require.NoError(t, err)
	...
	t.Run("error_case_type_cannot_be_found", func(t *testing.T) {
		...
		_, _, err = typeSystem.GetLowestWeightEdgeForExclusion("bad_label", "user")
		require.Error(t, err)
	})
