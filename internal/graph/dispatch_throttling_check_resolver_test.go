...
func TestDispatchThrottlingCheckResolver(t *testing.T) {
	...
	t.Run("above_threshold_throttle_but_timeout_while_waiting", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		...
		go func() {
			...
			//nolint:testifylint
			require.ErrorIs(t, err, errors.DispatchThrottleContextDeadline)
		}()
		...
		// we should not have executed the inner function because of timeout
		require.Equal(t, 0, resolveCheckDispatchedCounter)
	})

	t.Run("above_threshold_throttle_but_timeout_while_executing_the_function", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()
		...
		initialMockResolver.EXPECT().ResolveCheck(gomock.Any(), req).DoAndReturn(func(ctx context.Context, req *ResolveCheckRequest) (*ResolveCheckResponse, error) {
			resolveCheckDispatchedCounter++
			return nil, errors.RequestDeadlineExceeded
		}).Times(1)
		...
		go func() {
			...
			_, err := dut.ResolveCheck(ctx, req)
			//nolint:testifylint
			require.ErrorIs(t, err, errors.RequestDeadlineExceeded)
		}()
